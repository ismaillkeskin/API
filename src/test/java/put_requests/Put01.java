...
public class Put01  extends JsonPlaceHolderBaseUrl {
    ...
    public void put01() {
     ...
     Response response = given(spec).body(expectedData).when().put("{first}/{second}");
        //Serialization yaptik ==> Java datasini Json datasina cevirdik

       response.prettyPrint();

        //Do assertion
        Map<String,Object> actualData = response.as(Hashtable.class); //Burda Deserialization islemi yaptik
        System.out.println("Actual Data: "+actualData);

        //Status Code Dogrulamasi
        assertEquals(200,response.statusCode());

        //expectedData ile actualData birebi ayni degil. Bunlari tek tek ayirip degerleri karsilastiriyoruz
